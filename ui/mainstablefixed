--// Hydroxide UI - STABLE FIXED EDITION
--// File: ui/mainstablefixed.lua
--// Purpose: identical UI + safe thumbnail handling + no hangs
...
----------------------------------------------------------------
-- SAFE THUMBNAIL MANAGER (CRITICAL FIX)
----------------------------------------------------------------
local ThumbnailQueue = {}

for _, obj in ipairs(Interface:GetDescendants()) do
	if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
		if type(obj.Image) == "string" and obj.Image:find("rbxthumb://") then
			table.insert(ThumbnailQueue, {
				object = obj,
				image = obj.Image
			})
			-- prevent immediate fetch storm
			obj.Image = ""
		end
	end
end

-- load thumbnails gradually (1 per frame)
task.spawn(function()
